    ...
    public  InteractionResultHolder<ItemStack> use( Level pLevel,  Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (pUsedHand == InteractionHand.MAIN_HAND && !Screen.hasShiftDown()) {
            if (pPlayer instanceof ServerPlayer) {
                PatchouliAPI.get().openBookGUI((ServerPlayer) pPlayer, ItemRegister.make("fd_cookbook"));
...
//                    pContext.getPlayer().getUUID());
    ...
    public static  Component getEdition() {
        try {
            return PatchouliAPI.get().getSubtitle(ItemRegister.make("fd_cookbook"));
            //return new TextComponent("");
        } catch (IllegalArgumentException e) {
            return Component.empty(); // TODO Adjust Patchouli because first search tree creation is too early to get
            // the edition
        }
    }
